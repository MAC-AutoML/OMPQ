    ...
    def cfg2params_perlayer(self, cfg, length, quant_type):
        ...
        for i in range(4):
            ...
            if i != 0 and self.block == BasicBlock:
                params[count+1] += (inplanes * 1 * 1 * first_planes[-1] + 2 * first_planes[-1])  # downsample layer
            if self.block == Bottleneck:
                params[count + 2] += (inplanes * 1 * 1 * first_planes[-1] + 2 * first_planes[-1])  # downsample layer
            if self.depth != 50:
                params[count] += (inplanes * 3 * 3 * first_planes[0] + 2 * first_planes[0])
                params[count+1] += (first_planes[0] * 3 * 3 * first_planes[1] + 2 * first_planes[1])
                count += 2
            else:
                params[count] += (inplanes * 1 * 1 * first_planes[0] + 2 * first_planes[0])
                params[count+1] += (first_planes[0] * 3 * 3 * first_planes[1] + 2 * first_planes[1])
                params[count+2] += (first_planes[1] * 1 * 1 * first_planes[2] + 2 * first_planes[2])
                count += 3
            for j in range(1, self.blocks[i]):
                inplanes = first_planes[-1]
                if self.depth != 50:
                    params[count] += (inplanes * 3 * 3 * later_planes[j - 1] + 2 * later_planes[j - 1])
                    params[count+1] += (later_planes[j - 1] * 3 * 3 * inplanes + 2 * inplanes)
                    count += 2
                else:
                    params[count] += (inplanes * 1 * 1 * later_planes[2 * (j - 1)] + 2 * later_planes[2 * (j - 1)])
                    params[count+1] += (later_planes[2 * (j - 1)] * 3 * 3 * later_planes[2 * (j - 1) + 1] + 2 * later_planes[
                        2 * (j - 1) + 1])
                    params[count+2] += (later_planes[2 * (j - 1) + 1] * 1 * 1 * inplanes + 2 * inplanes)
                    count += 3
        if quant_type == 'PTQ':
            if self.depth == 50:
                params[count] += (cfg[2 * blocks[0] + 2 * blocks[1] + 2 * blocks[2] + 6] + 1) * self.num_classes
            else:
                params[count] += (cfg[blocks[0] + blocks[1] + blocks[2] + 5] + 1) * self.num_classes

        if quant_type == 'QAT':
            first_last_size += (3 * 7 * 7 * cfg[0] + 2 * cfg[0])
            if self.depth == 50:
                first_last_size += (cfg[2 * blocks[0] + 2 * blocks[1] + 2 * blocks[2] + 6] + 1) * self.num_classes
            else:
                first_last_size += (cfg[blocks[0] + blocks[1] + blocks[2] + 5] + 1) * self.num_classes

        return params, first_last_size

    def cfg2flops_layerwise(self, cfg, length, quant_type):  # to simplify, only count convolution flops
        blocks = self.blocks
        ...
        if quant_type == 'PTQ':
            flops[count] += (3 * 7 * 7 * cfg[0] * size * size + 5 * cfg[0] * size * size) # first layer, conv+bn+relu
            count += 1
        if quant_type == 'QAT':
            first_last_flops += (3 * 7 * 7 * cfg[0] * size * size + 5 * cfg[0] * size * size)  # first layer, conv+bn+relu
        inplanes = cfg[0]
        size /= 2 # pooling s=2
        ...
        for i in range(4): # each layer
            ...
            if self.block == BasicBlock and i != 0:
                flops[count+1] += (inplanes * 1 * 1 * first_planes[-1] + 5 * first_planes[-1]) * size * size # downsample layer
            elif self.block == Bottleneck:
                flops[count+2] += (inplanes * 1 * 1 * first_planes[-1] + 5 * first_planes[-1]) * size * size # downsample layer
            if self.depth != 50:
                flops[count] += (inplanes * 3 * 3 * first_planes[0] + 5 * first_planes[0]) * size * size
                flops[count+1] += (first_planes[0] * 3 * 3 * first_planes[1] + 5 * first_planes[1]) * size * size
                count += 2
            else:
                size *= 2
                flops[count] += (inplanes * 1 * 1 * first_planes[0] + 5 * first_planes[0]) * size * size
                size /= 2
                flops[count+1] += (first_planes[0] * 3 * 3 * first_planes[1] + 5 * first_planes[1]) * size * size
                flops[count+2] += (first_planes[1] * 1 * 1 * first_planes[2] + 5 * first_planes[2]) * size * size
                count += 3
            for j in range(1, self.blocks[i]):
                inplanes = first_planes[-1]
                if self.depth != 50:
                    flops[count] += (inplanes * 3 * 3 * later_planes[j-1] + 5 * later_planes[j-1]) * size * size
                    flops[count+1] += (later_planes[j-1] * 3 * 3 * inplanes + 5 * inplanes) * size * size
                    count += 2
                else:
                    flops[count] += (inplanes * 1 * 1 * later_planes[2*(j-1)] + 5 * later_planes[2*(j-1)]) * size * size
                    flops[count+1] += (later_planes[2*(j-1)] * 3 * 3 * later_planes[2*(j-1)+1] + 5 * later_planes[2*(j-1)+1]) * size * size
                    flops[count+2] += (later_planes[2*(j-1)+1] * 1 * 1 * inplanes + 5 * inplanes) * size * size
                    count += 3
        if quant_type == 'PTQ':
