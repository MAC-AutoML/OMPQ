            ...
            for i in range(n):
                ...
                if i == 0:
                    ...
                    if t==1:
                        ...
                        params[count+1] += (1 * 1 * hidden_dim * output_channel + 2 * output_channel)
                        count += 2
                    else:
                        params[count] += (1 * 1 * input_channel * hidden_dim + 2 * hidden_dim)
                        params[count+1] += (3 * 3 * hidden_dim + 2 * hidden_dim)
                        params[count+2] += (1 * 1 * hidden_dim * output_channel + 2 * output_channel)
                        count += 3
                else:
                    # self.features.append(block(input_channel, output_channel, 1, expand_ratio=t))
                    if t==1:
                        ...
                        count += 3
                input_channel = output_channel
        params[count] += (1 * 1 * input_channel * self.last_channel + 2 * self.last_channel)  # final 1x1 conv
        count += 1
        params[count] += ((self.last_channel + 1) * self.num_classes)  # fc layer
        return params, first_last_size

    def cfg2flops_layerwise(self, cfg, length, quant_type = 'PTQ'):  # to simplify, only count convolution flops
        interverted_residual_setting = [
            # t, c, n, s
            [1, cfg[1], 1, 1],
            [6, cfg[2], 2, 2],
        ...
        first_last_flops = 0.
        size = size//2
        flops[count] += (3 * 3 * 3 * cfg[0] + 0 * cfg[0]) * size * size # first layer
        count += 1
        input_channel = cfg[0]
        for t, c, n, s in interverted_residual_setting:
            output_channel = c
            for i in range(n):
                ...
                if i == 0:
                    ...
                    # self.features.append(block(input_channel, output_channel, s, expand_ratio=t))
                    if t==1:
                        flops[count] += (3 * 3 * hidden_dim + 0 * hidden_dim) * size * size
                        flops[count+1] += (1 * 1 * hidden_dim * output_channel + 0 * output_channel) * size * size
                        count += 2
                    else:
                        size = size * s
                        flops[count] += (1 * 1 * input_channel * hidden_dim + 0 * hidden_dim) * size * size
                        size = size // s
                        flops[count+1] += (3 * 3 * hidden_dim + 0 * hidden_dim) * size * size
                        flops[count+2] += (1 * 1 * hidden_dim * output_channel + 0 * output_channel) * size * size
                        count += 3
                else:
                    # self.features.append(block(input_channel, output_channel, 1, expand_ratio=t))
                    if t==1:
                        ...
                        count += 3
                input_channel = output_channel
        flops[count] += (1 * 1 * input_channel * self.last_channel + 0 * self.last_channel) * size * size # final 1x1 conv
        count += 1
        flops[count] += ((2 * self.last_channel - 1) * self.num_classes) # fc layer
